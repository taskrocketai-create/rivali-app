...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (places.length) return NextResponse.json({ provider: "openstreetmap", results: places.map((place) => ({ id: `osm-${place.place_id}`, label: place.display_name, latitude: Number(place.lat), longitude: Number(place.lon), type: place.type })) });
    ...
    return NextResponse.json({ provider: "photon", results });
  } catch (error) {
