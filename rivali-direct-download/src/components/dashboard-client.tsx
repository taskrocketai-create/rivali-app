...
export function DashboardClient({
  ...
  async function importStephenIntake() {
    setBusy(true);
    setMessage("");
    try {
      const response = await fetch("/api/fillout/stephen-intake/import", { method: "POST" });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error ?? "Could not import Stephen's intake.");
      setMessage(result.duplicate ? "Stephen's intake is already in Rivali." : "Stephen's pre-race baseline is now in Rivali. Refreshing the race shop…");
      window.setTimeout(() => window.location.reload(), 700);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Could not import Stephen's intake.");
      setBusy(false);
    }
  }
  return (
        ...
        {tab === "upload" && (
          <form className="card stack" onSubmit={uploadSession}>
            ...
            <div className="shortcut-setup">
              <div>
                <strong>Stephen’s saved pre-race intake</strong>
                <p>Pull the latest completed Fillout response into Rivali once, then attach his MyChron file to that baseline.</p>
              </div>
              <button className="button small secondary" type="button" disabled={busy} onClick={() => void importStephenIntake()}>
                Import Stephen’s intake
              </button>
            </div>
